...
let rec internal formatASTRec (node: AST.Node<'E,'T>): Tree =
    ...
    match node.Expr with
        ...
        | IntersectionDef(expr1, expr2) -> Node("IntersectionDef", [("Expr1", formatASTRec expr1); ("Expr2", formatASTRec expr2)])
        | AxiomDef(exprs, expr) -> 
            let exprNodes = List.map (fun node -> ("", formatASTRec node)) exprs
        ...
        | Not(expr) -> Node("Not", [("", formatASTRec expr)])
        | And(lhs, rhs) -> Node("And", [("LHS", formatASTRec lhs); ("RHS", formatASTRec rhs)])
        | Or(lhs, rhs) -> Node("Or", [("LHS", formatASTRec lhs); ("RHS", formatASTRec rhs)])
        | Let(name, init, scope) -> Node("Let", [("Name", Node(name, [])); ("Init", formatASTRec init); ("Scope", formatASTRec scope)])
        | If(cond, thenBranch, elseBranch) -> Node("If", [("Condition", formatASTRec cond); ("Then", formatASTRec thenBranch); ("Else", formatASTRec elseBranch)])
        | NotBelongs(expr1, expr2) -> Node("NotBelongs", [("LHS", formatASTRec expr1); ("RHS", formatASTRec expr2)])
        | Equals(expr1, expr2) -> Node("Equals", [("LHS", formatASTRec expr1); ("RHS", formatASTRec expr2)])
        | NotEquals(expr1, expr2) -> Node("NotEquals", [("LHS", formatASTRec expr1); ("RHS", formatASTRec expr2)])
        | LessThan(lhs, rhs) -> Node("LessThan", [("LHS", formatASTRec lhs); ("RHS", formatASTRec rhs)])
        | GreaterThan(lhs, rhs) -> Node("GreaterThan", [("LHS", formatASTRec lhs); ("RHS", formatASTRec rhs)])
        // | IfThenElse(cond, thenExpr, elseExpr) -> 
        //     Node("IfThenElse", [("Condition", formatASTRec cond); ("Then", formatASTRec thenExpr); ("Else", formatASTRec elseExpr)])
        | GreaterOrEqual(lhs, rhs) -> Node("GreaterOrEqual", [("LHS", formatASTRec lhs); ("RHS", formatASTRec rhs)])
        | LessOrEqual(lhs, rhs) -> Node("LessOrEqual", [("LHS", formatASTRec lhs); ("RHS", formatASTRec rhs)])
        | LessThan(lhs, rhs) -> Node("LessThan", [("LHS", formatASTRec lhs); ("RHS", formatASTRec rhs)])
        | GreaterThan(lhs, rhs) -> Node("GreaterThan", [("LHS", formatASTRec lhs); ("RHS", formatASTRec rhs)])
        | Assign(assignNode) -> 
